...
import DBClientsPageSkeleton from "@/components/skeletons/DBClientsPageSkeleton";

type Props = {};

export default function ClientPageComponent({}: Props) {
  const { isLoading, isError, data, isSuccess } = useReactQuery<Client[]>(
    "get-clients",
    "/clients"
  );

  if (isLoading) {
    return <DBClientsPageSkeleton />;
  }
          ...
          <div className="w-full max-w-[15rem] bg-white p-4 flex flex-col gap-6 rounded-lg">
            ...
            {isSuccess && (
              <p className="text-3xl font-semibold">
                <CountUp end={data.data.data.length} />
              </p>
            )}
          </div>
          <CreateClientModal>
            <button className="py-2 px-6 bg-green-dark rounded text-white">
              New Client
            </button>
          </CreateClientModal>
        </div>
